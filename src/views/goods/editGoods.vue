  ...
  <div class="createNewGoods">
    ...
    <div class="borderBox">
      ...
      >
        ...
        <div>
          ...
          <el-form-item label="商品轮播图" prop="file">
            <span>（默认第一张为商品封面图）</span>
              <img :src="item.imgUrl"  :key="item.qiniuId" v-for="item in productPicture">
            <el-upload
              v-if="isShowBanner"
              multiple
        ...
        <el-row class="butBox">
          <el-button type="danger" @click="submitToTheShelves">提交修改</el-button>
          <el-button plain  @click="saveDraft">取消编辑</el-button>
        </el-row>
        {{objInfo}}
      </el-form>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取商品参数并解析
      getGoods(){
        this.objInfo = this.$store.state.goodsInfo
        if(this.objInfo.productSpecOption.length != 0){
          this.isSpecifications = true
          this.isTips = true;
          this.objInfo.productSpecOption.forEach(item=>{
            if(item.optionImg !== ''){
        ...
        this.product = this.objInfo.product
        this.tableData = this.objInfo.productSku
        this.productPicture = this.objInfo.productPicture
        this.productSpecOption = this.objInfo.productSpecOption
      },
      // 获取所有分类
      getAll() {
